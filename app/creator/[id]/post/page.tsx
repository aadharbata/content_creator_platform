...
export default function CreatePost() {
  ...
  if (!creatorId) {
    return <div className="p-6 max-w-xl mx-auto text-red-600 font-bold">Invalid creator ID in URL.</div>;
  }
  ...
  return (
    <div className="p-6 max-w-xl mx-auto">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div
          className={`mt-4 flex flex-col items-center justify-center rounded-lg border-2 border-dashed ${dragActive ? 'border-indigo-600 bg-indigo-50' : 'border-gray-300 bg-white'} px-6 py-10 transition-all duration-300 ease-in-out cursor-pointer relative`}
          ...
          <div className="flex flex-wrap gap-4 my-4 justify-center">
            {previews.map((src, i) => (
              media[i]?.type.startsWith('image/') ? (
                <img key={i} src={src} alt="preview" className="w-28 h-28 object-cover rounded shadow" />
              ) : media[i]?.type.startsWith('video/') ? (
                <video key={i} src={src} controls className="w-28 h-28 object-cover rounded shadow" />
              ) : null
            ))}
          </div>
        )}
        <button type="submit" className="bg-blue-500 text-white px-6 py-2 mt-4 rounded font-semibold hover:bg-blue-600 transition-colors w-full">
          Post
        </button>
      </form>
    </div>
