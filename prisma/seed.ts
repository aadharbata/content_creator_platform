...
import { PrismaClient, Role, ContentType, ContentStatus, SubscriptionStatus, PaymentStatus, NotificationType } from '../lib/generated/prisma';
const prisma = new PrismaClient();

async function main() {
  // --- 1. Create Categories and Tags ---
  const categories = await Promise.all(
    ['Programming', 'Design', 'Marketing', 'Business', 'Music'].map(name =>
      prisma.category.create({ data: { name } })
    )
  );
  const tags = await Promise.all(
    ['TypeScript', 'React', 'Figma', 'SEO', 'Guitar'].map(name =>
  ...
  for (let i = 1; i <= 8; i++) {
    const user = await prisma.user.create({
      data: {
        ...
        role: i <= 3 ? 'CREATOR' : i == 8 ? 'ADMIN' : 'CONSUMER',
        StripeCustomerId: i % 2 === 0 ? `cus_${1000 + i}` : undefined,
      },
    });
    users.push(user);
    ...
    await prisma.userProfile.create({
      data: {
        bio: `Bio for User ${i}`,
        avatarUrl: `https://api.dicebear.com/7.x/identicon/svg?seed=${user.id}`,
        website: `https://user${i}.site`,
        twitter: `@user${i}`,
        instagram: `@insta${i}`,
        youtube: `user${i}YT`,
        userId: user.id,
  ...
  for (let i = 1; i <= 5; i++) {
    const course = await prisma.course.create({
      data: {
        ...
        price: 49.99 + i,
        duration: 10 + i,
        authorId: users[i % 3].id, // Assign to first 3 creators
        imgURL: `https://picsum.photos/seed/course${i}/400/200`,
        rating: 4.0 + (i % 2) * 0.5,
      },
    });
    courses.push(course);
  ...
  // --- 4. Create Content for Courses ---
  const contents = [];
  for (let i = 1; i <= 10; i++) {
    const content = await prisma.content.create({
      data: {
        title: `Content ${i}`,
        type: (['VIDEO', 'ARTICLE', 'PODCAST', 'EBOOK'] as ContentType[])[i % 4],
        url: `https://content.example.com/${i}`,
        status: (['PUBLISHED', 'DRAFT', 'ARCHIVED'] as ContentStatus[])[i % 3],
  ...
  for (let i = 0; i < 10; i++) {
    await prisma.lesson.create({
      data: {
        title: `Lesson ${i + 1}`,
        description: `Lesson ${i + 1} of Course ${courses[i % 5].title}`,
        order: (i % 3) + 1,
        contenturl: `https://lesson.example.com/${i + 1}`,
        courseId: courses[i % 5].id,
      },
    });
  }
  ...
  for (let i = 4; i < 8; i++) {
    const subscription = await prisma.subscription.create({
      data: {
        status: (['ACTIVE', 'CANCELLED', 'PAUSED', 'EXPIRED'] as SubscriptionStatus[])[i % 4],
        startdate: new Date(Date.now() - i * 86400000),
        endDate: i % 2 === 0 ? new Date(Date.now() + i * 86400000) : undefined,
        userId: users[i].id,
      },
    });
    await prisma.payment.create({
      data: {
        amount: 49.99 + i,
        currency: 'INR',
        status: (['SUCCEEDED', 'PENDING', 'FAILED', 'REFUNDED'] as PaymentStatus[])[i % 4],
        paymentvia: i % 2 === 0 ? 'card' : 'upi',
        paymentId: `pay_${2000 + i}`,
  ...
  for (let i = 0; i < 8; i++) {
    await prisma.review.create({
      data: {
        rating: (i % 5) + 1,
        comment: `Review ${i + 1} for course ${courses[i % 5].title}`,
        userId: users[i].id,
        courseId: courses[i % 5].id,
      },
    });
  ...
  for (let i = 0; i < 8; i++) {
    await prisma.notification.create({
      data: {
        type: (['SYSTEM', 'PAYMENT', 'CONTENT', 'SUBSCRIPTION'] as NotificationType[])[i % 4],
        message: `Notification ${i + 1} for User ${users[i].name}`,
        userId: users[i].id,
      },
    });
  }
  ...
  for (let i = 0; i < 5; i++) {
    const creator = users[i % 3];
    const fan = users[3 + (i % 5)];
    const conversation = await prisma.conversation.create({
      data: {
        ...
        messages: {
          create: [
            {
              content: `Hello from ${creator.name} to ${fan.name}`,
              senderId: creator.id,
            },
            {
              content: `Hi ${creator.name}, this is ${fan.name}`,
              senderId: fan.id,
            },
          ],
        },
  ...
  console.log('Seeded 20+ records successfully!');
}
