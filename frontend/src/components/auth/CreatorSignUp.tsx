...
import { Role } from "../../../../backend2/dist/lib/generated/prisma";
...
const CreatorSignUp = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log("Axios request code initiated");
      const res = await axios.post(
        // process.env.NEXT_PUBLIC_BACKEND_URL+"auth/creator-fan/signup",
        "http://localhost:5000/auth/creator-fan/signup",
        {
          name: form.name,
          email: form.email,
      ...
      // console.log("name: -", form.name, ", email: ,",form.email, ", password: ", form.password, ", role: ", form.role);
      // const res = await axios({
      //   url: "http://localhost:5000/auth/creator-fan/signup",
      //   method: "POST",
      //   headers: {
      //     "Content-Type": "application.json"
      //   },
      ...
      if (res.status === 500 || res.status === 400) {
        console.log("Status of signup of response: ", res.status);
