...
export default function TopCoursesSection({ language, translations, topCourses }: TopCoursesSectionProps) {
  ...
  const topCoursesText = language === 'hi' ? 'शीर्ष कोर्स' : 'Top Courses'
  const studentsText = language === 'hi' ? 'छात्र' : 'students'
  const viewAllText = language === 'hi' ? 'सभी देखें' : 'View All'
    ...
    <section className="py-20">
      <div className="container mx-auto px-4">
          ...
          {topCourses.map((course) => (
            ...
            >
              ...
              <CardContent className="p-6">
                ...
                <h3 className="font-bold text-lg mb-2 line-clamp-2 group-hover:text-blue-600 transition-colors dark:text-white">
                  {course.title}
                </h3>
                <p className="text-gray-600 dark:text-gray-300 mb-3">{course.author.name}</p>
                ...
                <div className="flex items-center justify-between text-sm text-gray-600 dark:text-gray-300">
                  <div className="flex items-center space-x-1">
                    ...
                    <span>
                      {(course.salesCount ?? 0).toLocaleString()} {studentsText}
                    </span>
                  </div>
                </div>
              </CardContent>
        ...
        <div className="text-center mt-12">
          <Button onClick={()=>router.push("/viewall")} size="lg" variant="outline" className="text-lg px-8 py-4 h-auto border-2 hover:bg-gray-50 dark:hover:bg-gray-700 dark:border-gray-600 dark:text-white">
            {viewAllText} {topCoursesText}
          </Button>
        </div>
      </div>
    </section>
